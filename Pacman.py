...
def background_video():
    HEIGHT = 600*SCALING
    WIDTH = 1066*SCALING
    frame_folder = "compressed_frames"
...
def move_player(player_pos, direction, input_direction, level):
    ...
    if (0 <= new_pos[0] < rows and 0 <= new_pos[1] < cols and level[new_pos[0]][new_pos[1]] not in [1, 5]):
        ...
        if (0 <= new_pos[0] < rows and 0 <= new_pos[1] < cols and level[new_pos[0]][new_pos[1]] not in [1, 5]):
            player_pos[:] = new_pos
#added
def check_player_collision(ghost_positions, player_pos, frightened_mode, level):
    got_hit = False
    ...
    return frightened_mode, got_hit
...
def read_level():
    with open('.\Karte\map1.json', 'r') as file:
        data = json.load(file)
    return data
...
def main():
    ...
    while running:
        ...
        #added frightened_mode turn off and on
        if frightened_mode:
        ...
        frightened_mode, got_hit = check_player_collision([ghost1_pos, ghost2_pos], player_pos, frightened_mode, level)
        if got_hit:
            got_hit = False
            player_lives -= 1
            print("live lost")
        ...
        #added chase and frightened_count zum reseten des cooldowns
        ...
        draw_text(str(score),font ,COLORS["coin"],screen, int(WIDTH - 60*SCALING),int(HEIGHT + 20*SCALING))
        pygame.display.flip()
